...
interface BoxBillTemplateProps {
  id?: string;
  className?: string;
  billData: BoxBillPrintData;
}

export function BoxBillTemplate({ id, className, billData }: BoxBillTemplateProps) {
  const { 
    id: billNo, 
    customerId, 
    customerName, 
  ...
  return (
    <div id={id} className={`box-bill-template-root ${className || ''}`}>
      ...
      <div className="bb-info-section">
        <div>
          <div><span className="bb-info-label">Id</span> <span className="bb-info-colon">:</span> <strong>{customerId}</strong></div>
          <div><span className="bb-info-label">Name</span> <span className="bb-info-colon">:</span> <strong>{customerName}</strong></div>
        </div>
