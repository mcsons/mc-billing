...
export default function PartyStatementPage() {
    const {
        parties, partyPayments,
        updatePartyPayment, softDeletePartyPayment,
        getPartyLedger,
    } = useData();
    ...
    // ── Party Statement ──────────────────────────────────────────────────────
    const [historySelectedPartyId, setHistorySelectedPartyId] = useState('');
    ...
    // ── Reactive statement ───────────────────────────────────────────────────
    // getPartyLedger is a useCallback whose ref changes whenever partyBills or
    // partyPayments change in DataContext. So this memo auto-recomputes on every
    // edit/delete. No extra Firestore read or listener is created here.
    const { transactions: filteredTransactions, openingBalance: openingBalanceForLedger } =
        useMemo(() => {
            if (!hasSearched || !historySelectedPartyId || !fromDate || !toDate) {
                return { transactions: [], openingBalance: 0 };
            }
            return getPartyLedger(historySelectedPartyId, { from: fromDate, to: toDate });
        }, [hasSearched, historySelectedPartyId, fromDate, toDate, getPartyLedger]);

    // ── Shared ReactSelect config ────────────────────────────────────────────
    const filterOption = (option: any, input: string) => {
        if (!input) return true;
        const isNumeric = /^\d+$/.test(input);
        if (isNumeric) return String(option.value) === input || String(option.value) === String(Number(input));
    ...
    const partyLabel = (p: { name: string; location?: string }) =>
        p.location ? `${p.name} (${p.location})` : p.name;

    const partyOptions = parties.map(p => ({ value: p.id, label: partyLabel(p) }));

    // ── Handlers ─────────────────────────────────────────────────────────────

    const handleSearchStatement = () => {
        if (!historySelectedPartyId) { toast({ variant: 'destructive', title: 'Party not selected' }); return; }
        if (!fromDate || !toDate) { toast({ variant: 'destructive', title: 'Date range not selected' }); return; }
        setHasSearched(true);
    };

    const handleClearSearch = () => {
        setHistorySelectedPartyId(''); setFromDate(undefined); setToDate(undefined); setHasSearched(false);
    };
    ...
    const openStatementPrint = (paper: 'thermal' | 'a4' | 'share') => {
        if (!historySelectedPartyId || (!filteredTransactions.length && openingBalanceForLedger === 0)) {
            toast({ variant: 'destructive', title: 'Nothing to Print', description: 'Please search for transactions first.' });
            return;
        }
        const party = parties.find(p => p.id === historySelectedPartyId);
        const printData = { party, transactions: filteredTransactions, openingBalance: openingBalanceForLedger, dateRange: { from: fromDate?.toISOString(), to: toDate?.toISOString() } };
        sessionStorage.setItem('partyStatementReportData', JSON.stringify(printData));

        if (paper === 'share') {
            window.open(`/print/party-statement?paper=a4&share=true`, '_blank');
        } else {
            window.open(`/print/party-statement?paper=${paper}`, '_blank');
        }
    };
    ...
    const historyPayments = useMemo(() => {
        ...
        if (historyFilterDate) {
            results = results.filter(p => {
                const pDate = (p.date as any)?.toDate ? (p.date as any).toDate() : new Date(p.date as any);
        ...
        return results.sort((a, b) => {
            const da = (a.date as any)?.toDate ? (a.date as any).toDate().getTime() : new Date(a.date as any).getTime();
            const db = (b.date as any)?.toDate ? (b.date as any).toDate().getTime() : new Date(b.date as any).getTime();
            return db - da;
        });
    ...
    const handleHistoryRowDoubleClick = (p: any) => {
        ...
        setHistorySelectedPartyId(p.partyId);
        setFromDate(day);
        setToDate(day);
        setHasSearched(true);
    ...
    const historySelectedParty = parties.find(p => p.id === historySelectedPartyId);
    const historyFilterParty = parties.find(p => p.id === historyFilterPartyId);

    return (
        <div className="flex flex-col gap-6 md:gap-8 w-full max-w-full overflow-x-hidden">
            <div className="grid grid-cols-1 gap-4 md:gap-8 items-start">
                {/* ── Party Statement ── */}
                <Card ref={statementRef as any}>
                    <CardHeader>
                        <CardTitle className="font-headline text-xl md:text-2xl">Party Statement</CardTitle>
                        <CardDescription>View a party's transaction history for a date range.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-4 md:p-6 space-y-5">
                        <div className="grid gap-2">
                            <Label>Party</Label>
                            <ReactSelect instanceId="history-party-select" placeholder="Select party..." isClearable
                                options={partyOptions}
                                value={historySelectedParty ? { value: historySelectedParty.id, label: partyLabel(historySelectedParty) } : null}
                                onChange={o => { setHistorySelectedPartyId(o ? o.value : ''); setHasSearched(false); }}
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                            <Button onClick={handleSearchStatement} className="w-full sm:w-auto h-11 text-base"><Search className="mr-2 h-4 w-4" /> Search</Button>
                        ...
                        <div className="max-h-72 overflow-y-auto">
                            ...
                            {/* Desktop Table - hidden on mobile via global CSS */}
                            <div className="payments-desktop-table overflow-x-auto">
                                <Table>
                                    ...
                                    <TableBody>
                                        {hasSearched && historySelectedPartyId ? (
                                            ...
                                            <TableRow><TableCell colSpan={7} className="h-24 text-center">Select a party and date range.</TableCell></TableRow>
                                        )}
                                    </TableBody>
                                </Table>
                            </div>

                            {/* Mobile Card List - hidden on desktop via global CSS */}
                    ...
                    <CardFooter className="flex-wrap gap-2">
                        <Button onClick={() => openStatementPrint('thermal')}>🧾 Print Receipt (106mm)</Button>
                        <Button variant="outline" onClick={() => openStatementPrint('a4')}>📄 Print Statement (145mm)</Button>
                        <Button variant="outline" onClick={() => openStatementPrint('share')} className="border-green-500 text-green-700 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-950">📤 Share PDF</Button>
                    </CardFooter>
                </Card>
            </div>
                    ...
                    <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end">
                        <div className="grid flex-1 gap-2">
                            ...
                            <ReactSelect instanceId="history-filter-party"
                                options={partyOptions}
                                value={historyFilterParty ? { value: historyFilterParty.id, label: partyLabel(historyFilterParty) } : null}
                                    ...
                                    historyPayments.map((p, i) => {
                                        ...
                                        return (
                                            <TableRow key={i} className="cursor-pointer hover:bg-muted/50" onDoubleClick={() => handleHistoryRowDoubleClick(p)}>
                                                ...
                                                <TableCell>{party?.name || p.partyId}</TableCell>
                                                <TableCell>
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div className="flex flex-col items-start gap-1.5">
                                                <span className="font-semibold text-sm">{party?.name || p.partyId}</span>
