...
import { Party, Transaction } from '@/lib/data';
...
function PrintPartyStatementContent() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('partyStatementReportData');
    if (stored) {
      try {
        ...
        router.push('/dashboard/party-statement');
      }
    } else {
      router.push('/dashboard/party-statement');
    }
    return () => {
      sessionStorage.removeItem('partyStatementReportData');
    };
  }, [router]);
  ...
  const handleSharePDF = async () => {
    const captureEl = document.getElementById('pdf-area-party-statement');
    ...
    try {
      ...
      // Party Billing paper — same 145mm x 210mm sheet the party bill prints on.
      const pdf = new jsPDF({ orientation: 'portrait', unit: 'mm', format: [145, 210] });
      ...
      const fileName = `MC_PartyStatement_${partyName}_${fromStr}.pdf`;
      const file = new File([pdfBlob], fileName, { type: 'application/pdf' });
      // Parties have no phone field in the data model, so share falls back to
      // the generic WhatsApp chooser.
      const phone = '';
      const waMessage =
      ...
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({
            title: `Statement: ${partyName} - M.C & SONS`,
            text: `Party Statement From: ${fromStr}, To: ${toStr} by M.C & SONS FISH COMPANY`,
            files: [file],
          });
    ...
    <div>
        ...
        <div className={`print-root ${paper}`}>
          <div id="print-area">
            ...
            <h2 className="text-lg font-semibold my-1 text-center">Party Statement</h2>
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '8px', borderBottom: '2px solid #333', paddingBottom: '6px' }}>
            ...
            <div style={{ fontSize: '13px', fontWeight: 'bold', marginTop: '5px' }}>Party Statement</div>
          </div>
        ...
        /* ===============================
          STATEMENT PRINT — Party Billing paper (145mm x 210mm)
          Page margin 5mm => 135mm printable width, matching the
          existing Party Bill print page exactly.
        ================================ */
        @media print {
          .print-root.a4 #print-area {
            /* @page already applies a 5mm margin; keep inner padding small so
               nothing is pushed past the 135mm printable width. */
            padding: 0;
            width: 135mm;
            max-width: 135mm;
          ...
          /* Fixed layout stops the Balance/Received column being pushed off
             the right edge on the narrower sheet. */
          ...
          .print-root.a4 .col-date     { width: 34%; text-align: left; }
          .print-root.a4 .col-billed   { width: 33%; text-align: right; white-space: nowrap; }
          .print-root.a4 .col-received { width: 33%; text-align: right; white-space: nowrap; }

          /* Totals box: keep it inside the printable width */
          .print-root.a4 .totals-section {
            width: 70mm;
            max-width: 70mm;
          }
          .print-root.a4 .summary-table { width: 100%; }
          .print-root.a4 .summary-value { white-space: nowrap; }
