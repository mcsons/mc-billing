...
function PrintPageContent() {
  ...
  const [billData, setBillData] = useState<PartyBoxBillPrintData | null>(null);
  const [isSharing, setIsSharing] = useState(false);
  const [shareError, setShareError] = useState<string | null>(null);
  const paper = searchParams.get('paper') || 'thermal';
  const autoShare = searchParams.get('share') === 'pdf';
  const hasAutoShared = React.useRef(false);

  useEffect(() => {
    const sessionData = sessionStorage.getItem('PartyBoxBillPrintData');
    if (sessionData) {
      try {
        setBillData(JSON.parse(sessionData));
        return;
      } catch (error) {
        console.error('Failed to parse Party Box Bill data from sessionStorage:', error);
      }
    }
  ...
  const handleSharePDF = useCallback(async () => {
    ...
    try {
      ...
      const fileName = `MC_PartyBoxBill_${billDateFormatted}.pdf`;
      ...
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({ title: `Party Box Bill: ${billDateFormatted} - M.C & SONS`, files: [file] });
  ...
  const { id: billNo, partyId, partyName, billDate, prevBalanceBox, todaysFishBox, totalBox, emptyBox, balanceBox, description, driverMobile, driverName, vehicleNo } = billData;
  ...
  const hiddenA4 = (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', margin: '8px 0', fontSize: '12px', borderBottom: '1px dashed #555', paddingBottom: '8px' }}>
            <div>
      {/* Full screen loading for PDF Generation */}
      {isSharing && autoShare && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background text-foreground">
          <Loader2 className="h-12 w-12 animate-spin text-primary mb-4" />
          <h2 className="text-2xl font-bold font-headline mb-2">Preparing PDF...</h2>
          <p className="text-muted-foreground">Please wait while your document is being generated.</p>
        </div>
      )}
              <div>
      {/* Full screen loading for PDF Generation */}
      ...
      )}<strong>Name</strong> : {partyName}</div>
        </div>
      ...
      )}<strong>Bill No</strong> : {billNo}</div>
              <div>
      {/* Full screen loading for PDF Generation */}
      {isSharing && autoShare && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background text-foreground">
      ...
      )}<strong>Bill Date</strong> : {format(parsedDate, 'dd-MM-yyyy')}</div>
        </div>
      </div>
      ...
      {isSharing && autoShare && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background text-foreground">
          ...
          <p className="text-muted-foreground">Please wait while your document is being generated.</p>
              ...
              <p className="font-semibold text-sm leading-tight">Your Party Box Bill is ready to share!</p>
        ...
        <div id="print-area">
          ...
          <div className="mb-2 text-[13px] font-mono leading-tight flex flex-col gap-1">
            <div className="flex justify-between items-start">
              <div className="flex"><span className="inline-block w-10">Id</span><span className="mr-1">:</span><strong>{partyId}</strong></div>
            ...
            <div className="flex flex-wrap justify-between items-start gap-x-2 gap-y-1">
              <div className="flex flex-1"><span className="inline-block w-10 shrink-0">Name</span><span className="mr-1 shrink-0">:</span><strong className="whitespace-normal break-words">{partyName}</strong></div>
...
export default function PartyPartyBoxBillPrintPage() {
  return (
