...
import { PartyBoxBillTemplate } from '@/components/dashboard/PartyBoxBillTemplate';
  ...
  const handleSharePDF = useCallback(async () => {
    ...
    try {
      ...
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({ title: `Party Box Bill: ${billDateFormatted}`, files: [file] });
  ...
  return (
    <div>
          ...
          return (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', margin: '8px 0', fontSize: '12px', borderBottom: '1px dashed #555', paddingBottom: '8px' }}>
                <div>
                  <div><strong>Id</strong> : {partyId}</div>
                  <div><strong>Name</strong> : {partyName}</div>
                </div>
      ...
      <div className={`print-root ${paper}`}>
        <PartyBoxBillTemplate billData={billData} />
      </div>
...
export default function PartyBoxBillPrintPage() {
  return (
